...
const labelMilestonePrefix = core.getInput('labelMilestonePrefix').toLowerCase().trim()

const orgs = core.getInput('orgs').split(',').map(i => i.toLowerCase().trim()).filter(i => Boolean(i))
let ignoreMilestones = core.getInput('ingoreMilestones').split(',').map(i => i.toLowerCase().trim()).filter(i => Boolean(i))
...
const renderIssuesPulls = (label, issues) => issues
  .filter((issue) => issue.labels.map(({name}) => name.toLowerCase()).indexOf(label.toLowerCase()) > -1)
  .map(({title, number, url, assignees, state, repository: {name: repo}}) =>
    `* ${checkbox(state)} ${title} ([${repo}#${number}](${url}))${assignees.length === 0 ? '' : ` - ${renderAssignees(assignees).join(', ')}`}`)

const renderLabels = ({issues, labels, pullRequests}) => labels
...
const isAllowedMilestone = ({title}) => ignoreMilestones
...
const milestoneLabels = ({milestones}) => milestones.filter(isAllowedMilestone)
...
const renderRepositories = async (orgs) => {
  ...
  const tasks = onlyLabels.map((label) => orgs.map((org) => ({org, label})))
    .reduce((prev, next) => [...prev, ...next], [])
    .map(async ({org, label}) =>
      await octokit
        .graphql(queries.findIssuesPulls,
          {search: `label:${label} user:${org}`}))

  const results = (await Promise.all(tasks))
    .reduce((items, {search: {edges}}) => [...items, ...edges], [])
    .map(({node}) => canonicalizeIssue(node))
    .reduce(({labels, issues, pullRequests}, node) => {
      return ({
        issues: node.url.indexOf('/issues/') > -1 ? [...issues, node] : issues,
        pullRequests: node.url.indexOf('/pull/') > -1 ? [...pullRequests, node] : pullRequests,
        labels: [...labels, ...node.labels],
      })
    }, {labels: [], issues: [], pullRequests: []})

  const filterIssue = ({title}) => ({labels}) => {
...
const writeMarkdown = (markdown) => new Promise((resolve, reject) => {
  fs.writeFile(outputFile, markdown, (err) =>
    err ? reject(err) : resolve())
})

if (orgs.length > 0) {
  renderRepositories(orgs)
    .then(writeMarkdown)
    .catch(console.error)
  return
}
